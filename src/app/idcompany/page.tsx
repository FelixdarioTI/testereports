...
const Modal: React.FC<ModalProps> = ({ onClose, onAddCard }) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    onAddCard(formData);
    setFormData({ title: '', CNPJ: '', RazaoSocial: '', companyId: '' }); // Limpa os campos do formulário
    onClose();
  };
...
const Card: React.FC<CardProps> = ({ title, CNPJ, RazaoSocial, onDelete, companyId }) => {
  const copyIDCompanystep = (id: string) => {
...
const Home: React.FC = () => {
  ...
  const filteredCards = cards.filter(
    (card) =>
      ...
      card.CNPJ.toLowerCase().includes(searchTerm.toLowerCase()) ||
      card.RazaoSocial.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <>
      ...
      <div className="flex justify-center mb-4">
        <input
          ...
          className="w-full max-w-md p-4 rounded-lg border-gray-300 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent dark:bg-gray-800 dark:text-white"
          value={searchTerm}
